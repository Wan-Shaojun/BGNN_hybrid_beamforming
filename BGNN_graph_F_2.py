...
########################################################################################################################基于codebook设计analog precoding vectors#####################################################################
...
class Analogue_update(nn.Module):                                                                                       #2*num_M -> 2*num_ant_per_chain
    def __init__(self, num_ant_per_chain, analogue_channel, num_M, operation = 'mean'):
        ...
        self.mlp = Seq(*[self.mlp, Seq(Lin(analogue_channel[len(analogue_channel) - 1], num_ant_per_chain).double().to(device =device_num), Soft().cuda())])
        self.weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.weight_1.data.fill_(1.0)

    def forward(self, C, F, F_0, codebook):
        batch_size = C.shape[0]
        num_rf = C.shape[2]
        proba_n_c_d = torch.zeros([batch_size, self.num_ant_per_chain, num_rf], dtype=torch.float32).to(device =device_num)
        F_temp_2 = torch.zeros([batch_size, self.num_ant_per_chain, num_rf], dtype=torch.complex128).to(device =device_num)
        C_norm = torch.sum(C, dim=1)
        Index = torch.zeros([batch_size, num_rf], dtype=torch.float64).to(device=device_num)
        ...
        data_output = self.mlp(data_input)
        for nr in np.arange(0, num_rf):
            proba_n_c_d[:, :, nr] = data_output[batch_size*nr : batch_size*(nr+1), :]
        max_index = torch.max(proba_n_c_d, dim=1)[1]
        Index = max_index.type(torch.float64)
        # normalization
        # F_temp = F_temp_1 + self.weight_1 * F_0
        for nb in np.arange(0, batch_size):
            for nr in np.arange(0, num_rf):
                index = max_index[nb, nr]
                F_temp_2[nb, :, nr] = codebook[:, index]
        F_4 = torch.cat([F_temp_2.real, F_temp_2.imag], dim=1)
        return F_4, proba_n_c_d, max_index
...
class BiGraph_net_F_2(nn.Module):
    def __init__(self, num_ant_per_chain, num_M, num_iter, P_pow, channel, codebook):                                            #__init__()写成__int__()会报错多余参数
        super(BiGraph_net_F_2, self).__init__()
        self.num_ant_per_chain = num_ant_per_chain
        self.num_M = num_M
        self.P_pow = P_pow
    ...
    def forward(self, H, F_1, num_rf, Index_Compare):
        ...
        for ni in np.arange(0, self.num_iter):
            C = self.C_func(D, B, H)
            F, proba_n_c_d, max_index = self.F_func(C, F, F_0, self.codebook)
        ...
        loss = self.CrossEntropyLoss(proba_n_c_d, Index_Compare)
        return F_2, V, max_index, D, loss
