...
########################################################################################################################digital precoding采取Zero Forcing Precoding############################################
...
class Digital_update(nn.Module):                                                                                        #2*num_M -> 2
    def __init__(self, digital_channel, num_M, operation = 'mean'):
        ...
        self.mlp = Seq(*[self.mlp, Seq(Lin(digital_channel[len(digital_channel) - 1], 1).double().to(device =device_num), Sigmoid().double().to(device =device_num))])
        # self.weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
    ...
    def forward(self, D, B, P_pow_normalized, D_0):
        ...
        num_user = B.shape[2]
        D_1 = torch.zeros([batch_size, num_user, 1], dtype=torch.float64).to(device =device_num)
        D_out = torch.zeros([batch_size, num_user, 1], dtype=torch.float64).to(device =device_num)
        B_norm = torch.sum(B, dim=1)
        ...
        for nu in np.arange(0, num_user):
            ...
            # data_input[batch_size * nu: batch_size * (nu + 1), :] = M_k1[:, nu, :]
        dat_output = self.mlp(data_input)
        for nu in np.arange(0, num_user):
            D_1[:, nu, :] = dat_output[batch_size*nu : batch_size*(nu+1), :]
        # normalized the power to satisfy the total power constraint
        # D_temp = D_1 + D_0
        D_temp = D_1
...
def generalBF_ZF_generate(D, H, F_1, num_ant_per_chain):                                                                                         #to generate digital beamforming matrix from D
    ...
    H_inv = torch.inverse(torch.matmul(H_eff, H_1) + (num_user / P_pow) * torch.eye(num_user, device=device_num))           ######MMSE/RZF
    ...
    for nb in np.arange(0, batch_size):
        p_vector = torch.sqrt(D[nb, :, :]).squeeze()
...
class BiGraph_net_ZF(nn.Module):
    ...
    def forward(self, H, F_1, num_rf):
        ...
        D = (self.P_pow/(num_user*self.num_ant_per_chain)) * torch.ones([batch_size, num_user, 1]).to(device =device_num)
        D_0 = D
...
def Sum_rate_cal(H, F, D, num_ant_per_chain, num_ant, num_ant_2):
    ...
    F_1, V = generalBF_ZF_generate(D, H_1, F_temp, num_ant_per_chain)
